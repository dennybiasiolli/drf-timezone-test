...
class TimezoneTestViewSet(viewsets.ModelViewSet):
    ...
    def group_by_year_and_quarter(self, request):
        ...
        qs = qs.annotate(year=ExtractYear('value_dt'), quarter=ExtractQuarter('value_dt')).values('year', 'quarter').order_by('year', 'quarter').distinct()
        data = []
        for item in qs:
            year = int(item['year'])
            quarter = int(item['quarter'])
            items = list(self.get_queryset().filter(value_dt__year=year, value_dt__quarter=quarter).order_by('value_dt').values())
            data.append({'year': year, 'quarter': quarter, 'items': items})
